#!/usr/bin/env sh

# Define each public key as a separate variable
KEY1="ssh-rsa  chris@solstice.lan"
KEY2="ssh-rsa  chris@durandal.local"
KEY3="ssh-rsa  chris@aletheia"

# Ensure the .ssh directory exists
mkdir -p ~/.ssh

# Ensure the authorized_keys file exists
touch ~/.ssh/authorized_keys

# Function to add a key if it doesn't already exist
add_key_if_not_exists() {
    KEY="$1"
    if ! grep -qF "$KEY" ~/.ssh/authorized_keys 2>/dev/null; then
        echo "$KEY" >> ~/.ssh/authorized_keys
        echo "\nAdded key: $KEY"
    else
        echo "\nKey already exists: $KEY"
    fi
}

# Add each key
add_key_if_not_exists "$KEY1"
add_key_if_not_exists "$KEY2"
add_key_if_not_exists "$KEY3"

# Set proper permissions
chmod 600 ~/.ssh/authorized_keys
chmod 700 ~/.ssh

echo "Public keys have been processed for the current user"
